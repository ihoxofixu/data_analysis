...
def give_book(cur, command):
    ...
    for row in cur.execute(f"""SELECT * FROM books WHERE book_id={command[1]} and returning_date is NULL;"""):
        ...
        return 0
    cur.executemany("INSERT INTO records (reader_id, book_id, taking_date, returning_date) VALUES (?, ?, date(\"now\"), ?);",
...
def take_book(cur, command):
    ...
    if not flag:
        ...
        return 0
    cur.execute(f"UPDATE records SET returning_date = date(\"now\") where book_id={command[1]} and returning_date is NULL;")
    con.commit()
    return 1
